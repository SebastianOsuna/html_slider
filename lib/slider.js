(function(global) {
    var Slider = function Slider (options) {
        ...
        var defaultOptions = function(opts) {
            ...
            opts.forRange = opts.forRange || Slider.DEFAULTS.options;
            return opts;
        };
        ...
        function prepareDOM (elem) {
            var wrapper = document.createElement("div"),
                ...
                second_knob = document.createElement("span"),
            ...
            elem._progress = 0;
            if( options.forRange ){
                elem._lower_progress = 0;
            }
            elem.getWrapper = function() { return wrapper; };
            elem.getKnob = function() { return knob; };
            if( options.forRange ) {
                elem.getSecondaryKnob = function() { return second_knob; };
                elem.setLowerProgress = function(val) {
                    this._lower_progress = val;
                    second_knob.style.left = val + "%";
                }
            }
            ...
            elem.setProgress = function(val) {
                this._progress = val;
                progress.style.width = (options.forRange? Math.max(0, this._progress - this._lower_progress ) : val) + "%";
            ...
            second_knob.className = "slider-knob secondary-knob";
        ...
        function setupEvents(elem) {
            ...
            var bar = elem.getBar(),
                knob = elem.getKnob(),
                secondary_knob = elem.getSecondaryKnob && elem.getSecondaryKnob(),
                ...
                knobmouselistener = function(event) {
                    ...
                    percent = Math.max( Math.min( percent, 100 ), elem._lower_progress );
